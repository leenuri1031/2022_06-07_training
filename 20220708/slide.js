...
const endScrollAnimation = (target,translateVal) => {
  target.animate(
    [
...
const endRight = () => {
  let rightEdge = slider.scrollWidth - slider.offsetWidth;
  // offsetWidth - 컨테이너 보이는 넓이
  ...
  if (slider.scrollLeft === rightEdge) {
    ...
    slider.scrollLeft += slider.offsetWidth - 158;
    // 그렇지 않다면 가로 스크롤바의 위치를 가로스크롤바 + 컨테이너 보이는 넓이 - 158 (슬라이드를 가운데로 하기 위한 숫자)
  }
};
...
const endLeft = () => {
  if (slider.scrollLeft === 0) {
    endScrollAnimation(10);
    // 만일 가로 스크롤바 위치가 0이 된다면 함수를 실행 시켜줌
  } else {
    slider.scrollLeft -= slider.offsetWidth - 150;
    // 그렇지 않다면 가로 스크롤바의 위치를 가로스크롤바 - 컨테이너 보이는 넓이 - (맨 처음에 위치할 수 있도록 하는 숫자)
  }
};
...
const timer = setInterval(slideFn, 1000);
